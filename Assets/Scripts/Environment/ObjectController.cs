...
public class ObjectController : MonoBehaviour {
	...
	void CreateSpecialObjectList(List<GameObject> gameObjectListToFill){
		...
		Object[] prefabs = Resources.LoadAll("Prefabs/SpecialObjects");
	...
	void CreateCompleteSpawnableList (List<SpawnableObject> spawnableListToFill){
		...
		Object[] specialPrefabs = Resources.LoadAll("Prefabs/SpecialObjects");
		...
		Object[] otherObjects = Resources.LoadAll("Prefabs/OtherObjects");
		for (int i = 0; i < otherObjects.Length; i++) {
			SpawnableObject spawnable = ( (GameObject)otherObjects[i] ).GetComponent<SpawnableObject>();
			spawnableListToFill.Add(spawnable);
		}
	}
	...
	public GameObject SpawnDefaultObject (Vector2 positionXZ, List<Vector2> specialPositions, int index) {
		...
		if( specialPositions.Contains(positionXZ) ){
			newObj.tag = "DefaultSpecialItem"; // change the tag name so that we know to spawn a special object from here later!
		}
	...
	public List<Vector2> GenerateOrderedDefaultObjectPositions (int numDefaultObjects, Vector3 distancePos){ //ORDERED BY DISTANCE TO PLAYER START POS
		...
		defaultPositions = UsefulFunctions.SortByNextClosest(defaultPositions, distancePosXZ);
	...
	public float GetMaxDefaultObjectColliderBoundXZ(){
		...
		Collider defaultCollider = InGameDefaultObject.GetComponent<Collider> (); //has to be in the scene, otherwise we can't get the bounds.
